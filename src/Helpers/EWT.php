...
class EWT implements EWTInterface
{
  ...
  public static function encode($data,
                                string $secret,
                                string $hash = 'sha256',
                                string $alg = 'aes-256-ctr',
                                ?string $iv = null): string
  {
    ...
    return static::base64url_encode(\json_encode($ewt, JSON_THROW_ON_ERROR));
  }
   ...
   */
  public static function decode(string $data, string $secret ): ?string
  {
    # Make a HASH of the secret
    $secret = \openssl_digest($secret,'sha256');

    $b64 = static::base64url_decode($data);
    $ewt = \json_decode($b64, true, 512, JSON_THROW_ON_ERROR);
   ...
   */
  private static function sign(mixed $data, string $secret, string $algo = 'sha256'): string
  {
